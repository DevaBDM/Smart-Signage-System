    ...
    case "$ctrl" in
        ...
        ddcutil)
            if ! command -v ddcutil &> /dev/null; then
                read -p "[5/8] Install ddcutil + i2c-tools for DDC/CI control? [y/N]: " dd_ans
                if [[ "$dd_ans" =~ ^[Yy]$ ]]; then
                ...
                echo "[5/8] ddcutil already installed."
            fi
            ;;
        xset)
            if ! command -v xset &> /dev/null; then
                read -p "[5/8] Install x11-xserver-utils (provides xset)? [y/N]: " xs_ans
                if [[ "$xs_ans" =~ ^[Yy]$ ]]; then
                ...
                echo "[5/8] xset already installed."
            fi
            ;;
        noop)
